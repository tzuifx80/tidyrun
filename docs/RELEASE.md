...
1. Update versions consistently in the three published packages (`@tidyrun/core`, `@tidyrun/cli`, `@tidyrun/plugin-sdk`) and the `tidyrun` wrapper.
2. Run `npm ci`, `npm run typecheck`, `npm test`, `npm run build`, and `npm audit --audit-level=high` on Windows, macOS, and Linux CI.
3. Run `npm pack --dry-run -w TidyRun` and inspect that the standalone CLI bundle contains only `bin/` and `dist/`; the wrapper must not rely on workspace-only `@TidyRun/*` packages after installation.
