...
export class LeanMcpServer {
  ...
  async handle(request: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    if (method === "initialize") return { jsonrpc: "2.0", id, result: { protocolVersion: "2024-11-05", capabilities: { tools: {} }, serverInfo: { name: "leanagent", version: "0.1.0" } } };
    ...
    if (method === "resources/list") return { jsonrpc: "2.0", id, result: { resources: [{ uri: "leanagent://repository/state", name: "Repository state", mimeType: "application/json" }, { uri: "leanagent://session/stats", name: "Latest session stats", mimeType: "application/json" }] } };
    if (method === "resources/read") {
      const uri = String(((request.params ?? {}) as { uri?: string }).uri ?? "");
      if (uri === "leanagent://repository/state") return { jsonrpc: "2.0", id, result: { contents: [{ uri, mimeType: "application/json", text: JSON.stringify(snapshotRepository(this.options.repository), null, 2) }] } };
      if (uri === "leanagent://session/stats") {
  ...
  private async callTool(name: string, args: Record<string, unknown>): Promise<Record<string, unknown>> {
    if (name === "leanagent_get_artifact") {
      const id = String(args.id ?? "");
      const record = this.store.get(id);
      return { content: [{ type: "text", text: record ? JSON.stringify(record, null, 2) : `unknown artifact ${id}` }], isError: !record };
    }
    if (name === "leanagent_search_artifact") {
      const hits = this.store.search(String(args.id ?? ""), String(args.term ?? ""));
      return { content: [{ type: "text", text: hits.join("\n") }] };
    }
    if (name === "leanagent_repository_state") return { content: [{ type: "text", text: JSON.stringify(snapshotRepository(this.options.repository), null, 2) }] };
    if (name === "leanagent_affected_tests") {
    ...
    if (name === "leanagent_session_stats") {
...
function toolDefinitions(): Array<Record<string, unknown>> {
  return [
    { name: "leanagent_get_artifact", description: "Get artifact metadata by id.", inputSchema: { type: "object", properties: { id: { type: "string" } }, required: ["id"] } },
    { name: "leanagent_search_artifact", description: "Search stored raw artifact lines.", inputSchema: { type: "object", properties: { id: { type: "string" }, term: { type: "string" } }, required: ["id", "term"] } },
    { name: "leanagent_repository_state", description: "Return a conservative local repository fingerprint.", inputSchema: { type: "object", properties: {} } },
    { name: "leanagent_affected_tests", description: "Select tests for changed files using the local repository index.", inputSchema: { type: "object", properties: { changed: { type: "array", items: { type: "string" } } }, required: ["changed"] } },
    { name: "leanagent_session_stats", description: "Return the most recent completed local session metrics.", inputSchema: { type: "object", properties: {} } },
  ];
}
