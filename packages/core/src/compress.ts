...
function compressPytest(output: string, maxChars: number): string {
  ...
  return clip(["pytest compressed", ...unique([...summary, ...failed]), ...windows].filter(Boolean).join("\n"), maxChars);
}
...
function compressVitest(output: string, maxChars: number): string {
  ...
  return clip(["vitest/jest compressed", ...summary, ...windows].filter(Boolean).join("\n"), maxChars);
}

function compressTsc(output: string, maxChars: number): string {
  const lines = output.split("\n");
  const summary = lines.filter((line) => /Found \d+ errors?|error TS\d+|warning TS\d+/i.test(line)).slice(-12);
  const windows = contextWindows(lines, (line) => /error TS\d+|(?:^|\s)error:/i.test(line), 1, 4, 80);
  return clip(["tsc compressed", ...summary, ...windows].filter(Boolean).join("\n"), maxChars);
}
...
function compressPackageManager(output: string, maxChars: number): string {
  ...
  return clip(["package manager output compressed", ...summary, ...windows].filter(Boolean).join("\n"), maxChars);
}

function compressCargo(output: string, maxChars: number): string {
  const lines = output.split("\n");
  const summary = lines.filter((line) => /test result:|Finished|error:|warning:|failures:/i.test(line)).slice(-20);
  const windows = contextWindows(lines, (line) => /^error\b|test .*\.\.\. FAILED|thread .* panicked|^failures:/i.test(line.trim()), 2, 10, 60);
  return clip(["cargo output compressed", ...summary, ...windows].filter(Boolean).join("\n"), maxChars);
}
...
function compressGo(output: string, maxChars: number): string {
  ...
  return clip(["go output compressed", ...summary, ...windows].filter(Boolean).join("\n"), maxChars);
}
...
function compressDiagnostics(output: string, maxChars: number): string {
  ...
  return clip(["diagnostic output compressed", ...summary, ...windows].filter(Boolean).join("\n"), maxChars);
}
...
function compressGeneric(output: string, maxChars: number): string {
  ...
  return clip(["generic output compressed", ...head, omitted ? `… ${omitted} non-diagnostic lines omitted …` : "", ...hits, ...tail].filter(Boolean).join("\n"), maxChars);
}
...
function clip(text: string, maxChars: number): string {
  ...
  const marker = "\n… [truncated; full output is recoverable]";
