...
export const genericAdapter: AgentAdapter = {
  ...
  detect: () => true,
  describe: () => "Use leanagent run -- <command>; captures stdout/stderr, compresses output, and persists artifacts.",
};
...
export const agentAdapters: AgentAdapter[] = [
  rulesAdapter("codex", "OpenAI Codex", ["AGENTS.md"], "Rules synchronization is supported. Native tool interception depends on the Codex host and is not assumed; use the generic wrapper for command interception."),
  {
    ...
    install: async () => ({ files: ["CLAUDE.md"], notes: ["Claude Code supports MCP servers. Add `leanagent mcp` through the documented Claude MCP configuration; terminal interception still uses the generic wrapper."] }),
  ...
  },
  {
    ...
    install: async () => ({ files: ["GEMINI.md"], notes: ["Gemini CLI exposes JSON stdin/stdout hooks. Configure BeforeTool/AfterTool/SessionStart to invoke `leanagent hook gemini`; command interception is limited to tool payloads exposed by the hook contract."] }),
  ...
  rulesAdapter("aider", "Aider", [".aider.conf.yml", ".aider.model.settings.yml"], "Configuration detection is supported; use leanagent run for command interception."),
  genericAdapter,
];
